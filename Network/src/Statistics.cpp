...
void Statistics::PrintAveDelay()
{
	for (int i = 0; i < pairs.size(); i++)
		...
		<< (double)(pairs[i]->_totalDelay) / pairs[i]->_arrived << " seconds" << endl;
}
...
void Statistics::PrintLossRate()
{
	for (int i = 0; i < pairs.size(); i++)
		cout << "\t(" << setw(3) << pairs[i]->_src << "," << setw(3) << pairs[i]->_dest << "): " 
		<< (double)(pairs[i]->_attempted - pairs[i]->_arrived) / pairs[i]->_attempted << "%" << endl;
}
...
void Statistics::PrintAllStats(vector<shared_ptr<NetworkNode>>& nodes)
{
	...
	cout << "- The overall average total delay for a packet (measured from source to destination): " << (double)(Packet::GetTotalDelay()) / Packet::GetTotalArrived() << endl;
